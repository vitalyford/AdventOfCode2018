...
class Entry implements Comparable<Entry>{
    ...
    String message;
    int  id;
    //<which_minute, how_many_times>
    HashMap<Integer, Integer> asleep;
    ...
    Entry(Date date, String message) {
        this.asleep  = new HashMap<>();
        this.date    = date;
        this.message = message;
    }
    
    public void addAsleepMinutes(int startMinute, int endMinute) {
        // insert all minutes from the input range into the hashmap
        for (int i = startMinute; i < endMinute; i++) {
            if (asleep.containsKey(i)) 
                asleep.replace(i, ((Integer)asleep.get(i)) + 1);
            else
                asleep.put(i, 1);
        }
    }
    ...
    public int compareTo(Entry e) {
        if (date.equals(e.date)) System.out.println("Hmm, the dates are the same " + date);
...
public class AdventOfCode2018Day4 {
    ...
    private static int searchGuard(ArrayList<Entry> guards, Entry e) {
        int i = 0;
        for (Entry guard : guards) {
            if (guard.id == e.id) return i;
            i++;
        }
        return -1;
    }
    ...
    public static void main(String args[]) throws ParseException {
        ...
        ArrayList<Entry> guards = new ArrayList<>();
        
        while (true) {
            String line = s.nextLine();
            if (line.equals("")) break; // if we reached the end of the input, then break from the infinite loop
            
            Matcher m = p.matcher(line);
            if (m.matches()) {
                DateFormat f = new SimpleDateFormat("yyyy-MM-dd HH:mm");
                Date date = f.parse(m.group(1));
                String message = m.group(2);
                entries.add(new Entry(date, message));
            }
            else { // just in case if something is wrong with the input formatting
                System.out.println("Hmm, this input did not match and was not processed: " + line);
            }
        }
        int startMinute = -1, endMinute = -1, currentGuardIndex = -1;
        while (!entries.isEmpty()) {
            Entry e = entries.poll();
            String[] splitted = e.message.split(" ");
            if (splitted[0].equals("Guard")) {
                e.id = Integer.parseInt(splitted[1].substring(1));
                currentGuardIndex = searchGuard(guards, e);
                if (currentGuardIndex == -1) {
                    guards.add(e);
                    currentGuardIndex = guards.size() - 1;
                }
            }
            else if (splitted[0].equals("falls")) {
                Calendar calendar = GregorianCalendar.getInstance();
                calendar.setTime(e.date);
                startMinute = calendar.get(Calendar.MINUTE);
            }
            else if (splitted[0].equals("wakes")) {
                Calendar calendar = GregorianCalendar.getInstance();
                calendar.setTime(e.date);
                endMinute = calendar.get(Calendar.MINUTE);

                guards.get(currentGuardIndex).addAsleepMinutes(startMinute, endMinute);
        ...
        Entry bestGuard = null;
        int max = -1;
        int maxValue = -1, piggyId = -1, piggyMinute = -1;
        for (Entry e : guards) {
            Set<Integer> keys = e.asleep.keySet();
            int sum = 0;
            for (Integer key : keys) {
                sum += e.asleep.get(key);
                if (e.asleep.get(key) > maxValue) {
                    maxValue = e.asleep.get(key);
                    piggyId = e.id;
                    piggyMinute = key;
                }
            }
        ...
        Set<Integer> keys = bestGuard.asleep.keySet();
        max = -1;
        int minute = -1;
        for (Integer key : keys) {
            if (bestGuard.asleep.get(key) > max) {
                max = bestGuard.asleep.get(key);
                minute = key;
            }
        }
        System.out.println("Answer to the first part: " + bestGuard.id * minute);
        
        System.out.println("Answer to the second part: " + piggyId * piggyMinute);
