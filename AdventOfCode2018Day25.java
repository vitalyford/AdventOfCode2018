...
class Point {
	...
	public boolean closeTo(Point p) {
		if (Math.abs(a - p.a) + Math.abs(b - p.b) + Math.abs(c - p.c) + Math.abs(d - p.d) <= 3) return true;
		return false;
	}
}
        	...
        	do {
	        	...
	        	for (int k = 0; k < cluster.size(); k++) {
	        		for (int j = i; j < c.size(); j++) {
		        		if (!cluster.contains(c.get(j)) && cluster.get(k).closeTo(c.get(j))) {
		        			c.get(j).group = group;
		        			cluster.add(c.get(j));
		        			found = true;
		        		}
